...
namespace MMOwningLauncher.Classes
{
    public class ShellEx
    {
        ...
        private static extern int SHGetImageList(
            int iImageList,
            ref Guid riid,
            out ImageFromFile.IImageList ppv);

        [DllImport("Shell32.dll")]
        public static extern int SHGetFileInfo(
            string pszPath,
            int dwFileAttributes,
            ref ImageFromFile.SHFILEINFO psfi,
            int cbFileInfo,
            uint uFlags);
        ...
        {
            var shinfo = new ImageFromFile.SHFILEINFO();
        ...
        private static IntPtr getIconHandleFromFilePathWithFlags(
            string filepath, IconSizeEnum iconsize,
            ref ImageFromFile.SHFILEINFO shinfo, int fileAttributeFlag, uint flags)
        {
